...
- 2026-09-03 — `spec-maker` skill added (PROP-20260903-03, `.claude/skills/spec-maker/SKILL.md`): writes a structured spec (Intent/Scope/Requirements/Given-When-Then Acceptance Criteria/Traceability) to `.ai/specs/<ID>-<slug>.md` for every approved `/discuss` proposal before implementation begins — wired into `discuss`'s "please execute" step (`.claude/skills/discuss/SKILL.md`). Also has a scan mode (batched-confirm) for retroactively speccing hand-crafted features that predate this system. Meant as the reference point a future test skill will cite via AC IDs. Verified by running scan mode for real against this repo (see next line).
- 2026-09-03 — `spec-maker` scan mode run for real (batch approved by user): wrote 4 retroactive specs to `.ai/specs/` for features built before `spec-maker` existed — roles/manager hierarchy (SPEC-20260903-01), LeaveType (SPEC-20260903-02), LeaveRequest (SPEC-20260903-03), and the landing navbar fix (SPEC-20260903-04, included at user's explicit request despite being a trivial fix). Each has numbered Requirements + Given/When/Then Acceptance Criteria reconstructed from actual current code behavior, with open questions noted where intent wasn't clear from the code alone (e.g. no manager-role constraint on `manager_id`, no defined owner for the Cancelled status transition).
- 2026-09-03 — `commit` skill extended (PROP-20260903-02, `.claude/skills/commit/SKILL.md`): added a security sweep before every commit/push (scans for private keys, cloud API key patterns, literal secrets, credential-shaped files), formalized commit messages as conventional-commit summary + plain-prose body, and added a push step (current branch, `origin` by default) that is private-repo-only unless the user explicitly approves pushing to a public one — visibility is re-checked fresh on every push, never cached. Also made "every skill file gets its own commit" a hard rule regardless of other grouping. Verified live: pushing this very change stopped and asked for confirmation, because `github.com/mhdhfz9/mycuti` is still public (confirmed via GitHub API) — user has chosen to flip it to private on GitHub before the first real push.
